...
def linear_regression(X, y, eps=0):
    ...
    identity_matrix = np.identity(len(M)) # should give an identity matrix of size X
    alpha = eps*identity_matrix
    M=M+alpha
...
def mean_squared_error(y1, y2):
    length = len(y1) # both y1 and y2 should be of the same length - I assume they are! 
    
    error_square_sum = sum((y1 - y2)**2) 
    ...
    return error_square_sum/length  # replace this with your answer.
